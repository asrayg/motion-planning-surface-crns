...
Formally, the probability that fewer than $k$ gradient reactions fire before the first agent reaction is $\Pr[\text{Binomial}(k, \lambda_g / (\lambda_g + \lambda_a)) < k]$, which is small for $\lambda_g \gg \lambda_a$.
...
\paragraph{Expected time.} The gradient phase fires at rate $\lambda_g = 1/\varepsilon$ per enabled adjacency. Filling $n - 1$ sites at rate $\lambda_g$ gives $\mathbb{E}[T_g] = O(n/\lambda_g) = O(n\varepsilon)$.
