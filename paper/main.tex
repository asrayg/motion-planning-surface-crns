...
We observe this empirically in the $10\times10$ instance of~\Cref{fig:trajectory-10x10}: the Manhattan distance from $(0,0)$ to $(9,9)$ is $18$, but the agent's path has length $19$. On larger grids and denser obstacle layouts, the sub-optimality can be larger.
...
\paragraph{Expected path length.} Each of the $L$ agent moves incurs a suboptimal step with probability at most $O(\varepsilon)$ independently (we formalize independence via an interlaced-Poisson argument in~\Cref{lem:poisson-race} below). Let $S$ be the number of suboptimal moves. Each suboptimal move extends the path by at most $2$ (since the agent must backtrack to rejoin a shortest path from its current site). So $L \leq d + 2S$, and $\mathbb{E}[L] \leq d + 2 \cdot \mathbb{E}[S] \leq d + 2L \cdot O(\varepsilon)$.
...
Wait, this is \emph{better} than claimed, not worse! Let me re-examine. The issue is that a fast gradient fires \emph{in rate-weighted time}, but wall-clock time is what we care about. With $\lambda_g = 1/\varepsilon$, each gradient-adjacency fires after Exp$(1/\varepsilon)$ time = mean $\varepsilon$ time. So the whole gradient phase takes $\varepsilon \cdot n$ expected time, which is indeed $O(n\varepsilon)$.

But the \emph{number} of reactions is $\Theta(n)$ — the number of events to fire is fixed.

Actually the cost is not in wall-clock time but in the number of reactions ``wasted'' before the agent makes progress. Let me re-state: \emph{expected number of reactions before termination is $O(n/\varepsilon + d)$}, and expected wall-clock time is $O(n\varepsilon + d)$. We report reactions as the natural measure.

More carefully: in our base construction, $\mathbb{E}[T] = O(n+d)$ counts reactions. In the modified construction, the gradient fires faster per agent step, so the gradient completes before the agent does many moves — hence number of gradient reactions is still $O(n)$, but the number per unit of agent progress is $O(1/\varepsilon)$. Summing over $d$ agent moves gives $O(n/\varepsilon)$ extra gradient reactions, plus $O(d)$ agent moves themselves.
...
The base construction does not guarantee shortest paths. \Cref{fig:path-optimality} shows the empirical ratio $L/d$ (path length divided by shortest-path distance) across all trials. On obstacle-free grids, the median ratio is $\approx 1.10$; on dense-obstacle instances, it rises to $\approx 1.15$. The ratio remains bounded by a small constant across all instances we tested.
